...
## 7. 단계별 구축 전략: CloudWatch 중심 AIOps 우선 구현 및 GuardDuty 확장 로드맵

현재 TVING 플랫폼의 핵심 운영 요구사항과 기구축된 인프라 현황을 반영하여, **Phase 1에서 CloudWatch의 오탐/미탐 정제 AIOps를 우선 완성하고, Phase 2에서 GuardDuty를 도입**하는 단계적 구현 전략을 채택합니다.

```text
┌────────────────────────────────────────────────────────────────────────────────────────┐
│ [Phase 1: 현재 최우선 완성] CloudWatch 기반 오탐/미탐 정제 & SRE AIOps                 │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 1. 실시간 메트릭 수집: CloudWatch Metric Anomaly Detection (CPU/Memory 1분 주기)       │
│ 2. Bedrock AIOps 추론 엔진:                                                            │
│    • 【오탐 정제】: 신작 공개 대량 트래픽(Flash Crowd)을 DDoS로 오판하지 않고 SRE 확장    │
│    • 【미탐/원인분류】: 동일한 RDS CPU 95% 증상의 3대 원인(인기작/버그/DoS) 교차 검증    │
│ 3. 자율 대응 & 알림:                                                                   │
│    • ECS Fargate 태스크 증설 (1대 ➔ 4대) 및 캐시/DB 풀 안정화                          │
│    • Lambda (tving-aiops-slack-notifier) ➔ Slack 🚨 장애 경보 / ⚡ SRE 스케일아웃 보고│
└───────────────────────────────────────────┬────────────────────────────────────────────┘
                                            │ (Phase 1 안정화 후 확장)
                                            ▼
┌────────────────────────────────────────────────────────────────────────────────────────┐
│ [Phase 2: 향후 확장 도입] GuardDuty 기반 지능형 보안 침해 대응 (SOAR)                  │
├────────────────────────────────────────────────────────────────────────────────────────┤
│ 1. 위협 인텔리전스 수집: GuardDuty Finding (SSRF, 비정상 PortProbe, IAM 키 유출 등)   │
│ 2. SecAIOps 킬체인 종합 추론: 분산된 LOW~MEDIUM 이벤트를 종합하여 킬체인 공격 확정    │
│ 3. SOAR 자율 격리: 공격자 IP WAF 영구 차단, 침해 ECS 태스크 교체, 침해 세션 무효화  │
└────────────────────────────────────────────────────────────────────────────────────────┘
```

### 🎯 CloudWatch 우선 구축이 가장 타당한 3가지 핵심 이유

1. **기구축 파이프라인의 즉각적 활용성**:
   * 이미 CloudWatch Anomaly Detection 알람, SNS 토픽(`tving-aiops-anomaly-alarm`), Lambda 알림(`tving-aiops-slack-notifier`), Bedrock Harness 도구가 완벽히 연동되어 있어, **오탐/미탐 정제 추론 로직만 결합하면 즉시 실전 데모가 완성**됩니다.
2. **TVING OTT 비즈니스의 핵심 가치 직결**:
   * OTT 서비스에서 가장 빈번하고 치명적인 장애는 **"신작 공개 시 트래픽 폭증(Flash Crowd)"**과 **"동일 CPU 95% 발생 시의 정확한 원인 규명"**입니다. CloudWatch의 단순 수치 알람에 AIOps를 결합함으로써 비즈니스 다운타임을 방지하는 효과가 가장 극대화됩니다.
3. **위협 모델의 점진적 고도화**:
   * 인프라의 성능/가용성 관제(Phase 1)를 탄탄히 다진 후, 계정/네트워크 지능형 침해 대응(Phase 2)으로 범위를 확장하는 것이 **안정적이고 설득력 있는 엔지니어링 표준 로드맵**입니다.
