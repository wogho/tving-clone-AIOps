...
# ----------------------------------------------------------------------
# 방안 1: Bedrock Agent Direct Invoke (invoke_agent) 테스트 엔드포인트
# ----------------------------------------------------------------------

class OpsDirectAgentChatRequest(BaseModel):
    message: str
    agent_id: Optional[str] = "TSTALIASID"
    agent_alias_id: Optional[str] = "TSTALIASID"
    session_id: Optional[str] = None
    region: Optional[str] = "ap-northeast-2"


@router.post("/ai-chat-direct")
def ops_ai_chat_direct(request: OpsDirectAgentChatRequest):
    """
    [방안 1] 콘솔 Bedrock Agent(Harness) 직접 호출 엔드포인트
    ops.py 코드 내 도구 정의 없이 콘솔 에이전트를 실시간으로 직접 실행하여 결과를 반환합니다.
    """
    agent_id = request.agent_id.strip() if request.agent_id else ""
    agent_alias_id = request.agent_alias_id.strip() if request.agent_alias_id else "TSTALIASID"
    session_id = request.session_id if request.session_id else f"session-{int(time.time())}"
    region = request.region if request.region else "ap-northeast-2"

    if not agent_id or agent_id == "TSTALIASID":
        # 에이전트 ID 미지정 시 기본 안내 반환
        return {
            "reply": "⚠️ 콘솔의 Bedrock Agent ID가 설정되지 않았습니다.\n상단 입력창에 AWS 콘솔 Bedrock Agent ID(예: `CW9N0QAOGB` 또는 에이전트 ID)를 입력하고 다시 질문해 주세요.",
            "status": "need_agent_id",
            "tools_used": [],
            "agent_id": agent_id,
    ...
    try:
        ...
        if event_stream:
            for event in event_stream:
                if "chunk" in event:
                    ...
                    if chunk_bytes:
                        full_reply += chunk_bytes.decode("utf-8", errors="replace")
                elif "trace" in event:
                    trace_obj = event["trace"].get("trace", {})
                    # 도구 호출(Action Group) 추적
                    orch_trace = trace_obj.get("orchestrationTrace", {})
                    if "invocationInput" in orch_trace:
                        action_input = orch_trace["invocationInput"].get("actionGroupInvocationInput", {})
                        if action_input:
                            tools_used.append({
                                "tool": action_input.get("function", action_input.get("actionGroupName", "tool")),
                                "input": action_input.get("parameters", {}),
                                "output_summary": "Action Group Invocation via Bedrock Agent"
                            })

        return {
            "reply": full_reply if full_reply else "에이전트로부터 응답을 수신하지 못했습니다.",
            "status": "success",
        ...
        return {
            "reply": f"❌ Bedrock Agent 직접 호출 중 오류 발생: {str(e)}\n\n(입력된 Agent ID: `{agent_id}`, Alias ID: `{agent_alias_id}`, Region: `{region}`을 확인하세요.)",
            "status": "error",
            "error_detail": str(e),
            "tools_used": []
        }
